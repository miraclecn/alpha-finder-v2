...
class PortfolioPromotionGateEvaluatorTest(unittest.TestCase):
    ...
    def test_gate_fails_when_regime_overlay_inputs_are_incomplete(self) -> None:
        gate = load_promotion_gate(
            CONFIG_ROOT / "promotion_gates" / "a_share_core_portfolio_gate.toml"
        )
        evaluator = PortfolioPromotionGateEvaluator(gate)
        ...
        decision = evaluator.evaluate(
            SleevePromotionSnapshot(
                ...
                regime_overlay_blocked_periods=1,
                regime_overlay_missing_inputs=["price_limit_stress"],
            )
        )

        self.assertFalse(decision.passed)
        self.assertIn("regime_overlay_blocked", decision.failed_checks)
        self.assertIn("regime_overlay_missing_inputs", decision.failed_checks)
